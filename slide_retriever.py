...
class IndexCache:
    def __init__(self, index_dir):
        ...
        self.slides = json.loads(
            (index_path / "slides.json").read_text(encoding="utf-8")
        )

        self.vectorizer = joblib.load(
            index_path / "tfidf_vectorizer.joblib"
        )

        self.tfidf_matrix = joblib.load(
            index_path / "tfidf_matrix.joblib"
        )

        self.embedding_matrix = np.load(
            index_path / "embedding_matrix.npy"
        )

        embedding_meta = json.loads(
            (index_path / "embedding_meta.json").read_text(
                encoding="utf-8"
            )
        )

        self.embedding_model = embedding_meta["model"]
...
def get_hybrid_results(
    ...
    embedding_scores = embedding_matrix @ query_vector
    embedding_indices = np.argsort(embedding_scores)[::-1][:embedding_k]
